...
void display()
{
	...
	if (numused) {
		...
		for (int i = 0; i < numused*4; i += 4)
		{
			...
			GLfloat transTemp[4];
			temp[0] = lightposn[i];
			temp[1] = lightposn[i+1];
			temp[2] = lightposn[i+2];
			temp[3] = lightposn[i+3];
			transformvec(temp, transTemp);
			lightransf[i]	= transTemp[0];
			lightransf[i+1] = transTemp[1];
			lightransf[i+2] = transTemp[2];
			lightransf[i+3] = transTemp[3];
		}
	...
	for (int i = 0; i < numobjects; i++) {
		...
		const GLfloat shin[] = { obj->shininess };
		glUniform1fv(shininesscol, 1, shin);
